<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">

<head>
	<!-- metas -->
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />	
	<title>Rodrigo Maroto Portfolio 2015</title>
	<meta name="author" content="Rodrigo Maroto" />
	<meta name="description" content="portfolio 2013 Rodrigo Maroto" />
	<meta name="keywords"  content="design, mobile, tablet, ipad, iphone, android" />
	<meta name="Resource-type" content="Document" />

	<!-- CSS -->
	<!-- Less -->
	<script type="text/javascript">
		// Replace that on final version with CSS compiled version
	    less = { 
	        env: "development", // or "production"
	        async: true,       // load imports async
	        fileAsync: false,   // load imports async when in a page under
	                            // a file protocol
	        poll: 1000,         // when in watch mode, time in ms between polls
	        functions: {},      // user functions, keyed by name
	        dumpLineNumbers: "comments", // or "mediaquery" or "all"
	        relativeUrls: true,// whether to adjust url's to be relative
	    };
	</script>

	<link rel="stylesheet/less" type="text/css" href="css/styles.less" />

	<script type="text/javascript" src="js/less-1.5.0.min.js"></script>

	<!-- js -->
	<!--[if IE]>
		<script type="text/javascript">
			 var console = { log: function() {} };
		</script>
	<![endif]-->
	<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js"></script>
	<script src="http://ajax.googleapis.com/ajax/libs/jqueryui/1.9.1/jquery-ui.min.js"></script>	
	<script type="text/javascript" src="js/jquery.fullPage.js"></script>
	<script type="text/javascript">
		$(document).ready(function() {
			var pepe = $.fn.fullpage({
				slidesColor: ['#222', '#bb112b', '#004EA7', '#F5F5F5', '#3AA8D7', '#230B87', '#FF5600', '#900407', '#1D5EA0', '#222'],
				anchors: ['Hi!', 'IberiaExpress', 'GO!Chat', 'TouristEye', 'Tactilapp', '900', 'Destinia', 'Monomusic', 'ResiduosElectronicos', 'Contact'],
				menu: '#menu',
				easing: 'easeOutExpo'
			});
			
		});
	</script>
	<!-- Google Analytics -->
	<script>
	  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
	  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
	  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
	  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');
	
	  ga('create', 'UA-45846137-1', 'maroto.me');
	  ga('send', 'pageview');
	
	</script>

</head>
<body>

<!-- Intro -->
<div class="section " id="section0">
	<h1>Rodrigo Maroto</h1>
	<h2>UI & UX DESIGNER</h2>
	<p class="icons">
		<span>
			<a href="http://es.linkedin.com/in/rmaroto" target="_blank">
				<img src="img/ic_linkedin.png" alt="ic_linkedin" width="32" height="32">
			</a>
		</span>
		<span>
			<a href="http://dribbble.com/marotorod" target="_blank">
				<img src="img/ic_dribbble.png" alt="ic_dribbble" width="32" height="32">
			</a>
		</span>
		<span>
			<a href="http://www.behance.net/a1tinta" target="_blank">
				<img src="img/ic_behance.png" alt="ic_behance" width="32" height="32">
			</a>
		</span>
	</p>
	<p>
		This portfolio is always in progress. You can see the project on <a href="https://github.com/marotorod/marotorod.github.io" target="_blank">Github</a>
	</p>
	<p class="scroll">scroll or ▼▲ to continue</p>
</div>




<!-- Iberia Express -->
<div class="section " id="section1">
	<h1><img src="img/img_iberiaexpress_safari.png" alt="Iberia Express"></h1>
	<p>
		<h2>Web Manager at Iberia Express</h2>
		<ul>
			<li>Architecture website</li>
			<li>Define funcionalities</li>
			<li>UI & UX coordinate</li>		
			<li>Coordination production deployment</li>
			<li>Manage content</li>			
		</ul>
	</p>
</div>



<!-- Go!Chat -->
<div class="section " id="section2">
	<h1><img src="img/img_logo_gochat.png" alt="img_logo_gochat" width="108" height="70"></h1>
	<p>
		<img src="img/img_gochat_ui.png" alt="img_gochat_ui">
	</p>
	<p>
		<h2>ui designer</h2>
		<ul>
			<li>UI, export  and documentation</li>
		</ul>
	</p>
</div>



<!-- Touristeye -->
<div class="section " id="section3">
	<h1><img src="img/img_logo_touristeye.png" alt="img_logo_touristeye" width="268" height="50"></h1>
	<p>
		<img src="img/img_touristeye.png">
	</p>
	<p>
		<h2>ui designer</h2>
		<ul>
			<li>export & optim resources for apps</li>
			<li>design some company web layouts</li>
		</ul>
	</p>
</div>



<!-- Tactilapp -->
<div class="section " id="section4">
	<h1><img src="img/img_logo_tactilapp.png" alt="img_logo_tactilapp" width="258" height="78"></h1>
	<p>
		<img src="img/img_tactilapp.png">
	</p>
	<p>
		<h2>entrepreneur</h2>
		<ul>
			<li>brand manager</li>
			<li>UI & UX</li>
			<li>marketing</li>
		</ul>
	</p>
</div>



<!-- Novecientos -->
<div class="section " id="section5">
	<h1><img src="img/img_logo_novecientos.png" alt="img_logo_novecientos" width="203" height="129"></h1>
	<p>
		<img src="img/img_novecientos_app.png" alt="img_novecientos_app">
	</p>
	<p>
		<h2>UI & UX designer</h2>
		<ul>
			<li>UI & UX</li>
			<li>resources</li>
			<li>marketing</li>
		</ul>
	</p>
</div>



<!-- Destinia -->
<div class="section " id="section6">
	<h1><img src="img/img_logo_destinia.png" alt="img_logo_destinia" width="278" height="82"></h1>
	<p>
		<img src="img/img_destinia.png">
	</p>
	<p>
		<h2>concept ui designer</h2>
		<ul>
			<li>concept app for destinia.com</li>
		</ul>
	</p>
</div>



<!-- Monomusic -->
<div class="section " id="section7">
	<h1><img src="img/img_logo_monomusic.png" alt="img_logo_monomusic" width="394" height="110"></h1>
	<p>
		<img src="img/img_monomusic.png">
	</p>
	<p>
		<h2>designer</h2>
		<ul>
			<li>planning</li>
			<li>resources</li>
		</ul>
	</p>
</div>



<!-- Residuos Electronicos -->
<div class="section " id="section8">
	<h1><img src="img/img_logo_residuos_electronicos.png" alt="img_logo_residuos_electronicos" width="375" height="21"></h1>
	<p>
		<img src="img/img_residuos_electronicos.png">
	</p>
	<p>
		<h2>marketing manager</h2>
		<ul>
			<li>brand manager</li>
			<li>marketing</li>
			<li>web</li>
			<li>design</li>
		</ul>
	</p>
</div>



<!-- Contact -->
<div class="section " id="section89">
	<h1>Contact me!</h1>
	<p>
		If you like my work, you can send me an email at <a href="marotorod@gmail.com">marotorod@gmail.com</a> or call me at <a href="tel:+34626857577">+34 626 857 577</a>
	</p>
</div>


</body>
</html>
